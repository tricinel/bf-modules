...
class content extends Admin_Controller {
	...
	public function __construct() 
	{
		...
		$this->auth->restrict('Coupon.Content.View');
	...
	*/
	...
	{
		$this->auth->restrict('Coupon.Content.Create');
		...
		if ($this->input->post('submit'))
		{
			if ($insert_id = $this->save_coupon())
			{
				...
				Template::redirect(SITE_AREA .'/content/coupon');
			}
			else 
			{
	...
	*/
	...
	{
		$this->auth->restrict('Coupon.Content.Edit');
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/content/coupon');
		}
	...
	*/
	...
	{	
		$this->auth->restrict('Coupon.Content.Delete');
	...
	*/
	...
	{	
		...
		$coupon_uses_limit                 = $this->input->post('coupon_uses_limit');
		$coupon_uses_per_customer          = $this->input->post('coupon_uses_per_customer');
		$coupon_expiration_date            = $this->input->post('coupon_expiration_date');
		...
		$data['coupon_uses_limit']         = (empty($coupon_uses_limit)) ? null : $coupon_uses_limit;
		$data['coupon_uses_per_customer']  = (empty($coupon_uses_per_customer)) ? null : $coupon_uses_per_customer;
		$data['coupon_allowed_for_guests'] = $this->input->post('coupon_allowed_for_guests');
		$data['coupon_expiration_date']    = (empty($coupon_expiration_date)) ? null : $coupon_expiration_date;
		
		if ($type == 'insert')
		{
			$data['coupon_times_used'] = '0';
